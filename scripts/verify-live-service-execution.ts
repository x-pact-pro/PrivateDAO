...
const API_CHECKS: ApiCheck[] = [
  ...
  },
  {
    ...
    validate: (payload) => {
      ...
      if (!Array.isArray(payload?.mustPass) || !payload.mustPass.some((entry: string) => entry.includes("public attestation"))) {
        return "privacy execution claims must separate public attestation from private disclosure";
      }
